...
class prop(nn.Module):
    ...
    def forward(self,x,adj):
        # torch.eye 须与 adj 同设备，to(adj.device) 兼容 cpu/cuda/mps
...
class graph_constructor(nn.Module):
    ...
    def forward(self, idx):
        ...
        # mask 与 adj 同设备，兼容 cpu/cuda/mps（不依赖构造时的 self.device）
...
class graph_global(nn.Module):
    def __init__(self, nnodes, k, dim, device, alpha=3, static_feat=None):
        ...
        self.nnodes = nnodes
        self.A = nn.Parameter(torch.randn(nnodes, nnodes, device=device), requires_grad=True)
